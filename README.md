...
- **Папка правил `.claude rules/`** — рабочие соглашения по проекту
  (документация, git, секреты, стиль). Хранится **только локально** и
  занесена в `.gitignore` (на GitHub не уходит).
- **Чистка кредов в `config/systems.json`** — RTSP-логины/пароли iPanda
  Офиса и Нового цеха переехали в `.env`. В `systems.json` остались только
...
|---|---|---|
| `node src/index.js --light` | каждые 15 мин, круглосуточно (24/7) | статусы + timeline + `reports/live.html` |
