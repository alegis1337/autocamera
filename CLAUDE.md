...
## Обновление 8 июня 2026 — изоляция скриншотов в групповых письмах

Email-отчёты рассылаются раздельно по группам (Европласт / Онлайн), но
inline-миниатюры камер (CID-вложения) собирались **одним общим списком**
на обе группы и целиком прицеплялись к каждому письму. HTML-тело письма
фильтровалось по группе правильно (`buildReport({ group })`), а вот
вложения — нет: к письму «Европласт» прицеплялись и снимки камер «Онлайн»,
и наоборот. Почтовик 1С показывает не привязанные к телу inline-вложения
отдельной галереей, поэтому заказчик видел чужие скриншоты («у Онлайна
скриншоты Европласта, у Европласта — Онлайна»).

Исправлено в `src/index.js`: CID-снимки теперь собираются `buildSnapMap`
**отдельно по системам каждой группы** внутри цикла по `REPORT_GROUPS`.
И `snapMap` для `buildReport` (тело письма), и `cidList` для `sendReport`
(inline-вложения) берутся из одного группового набора — письмо каждого
заказчика содержит строго свои камеры. Глобальные `cidSnapMap`/`cidList`
удалены. Браузерный полный отчёт и `reports/live.html` не тронуты — это
внутренние виды, им положено показывать все системы (они на общем
`fileSnapMap`). Helpdesk-письма текстовые, без снимков, фикс их не касается.

Версия не поднята — это фикс: видимое поведение приведено к уже
задокументированному (письма раздельно по группам), новой функциональности нет.

---

## Обновление 31 мая 2026 — круглосуточный Light-мониторинг

`config/schedule.json`: `light.durationHours` поднят с 14 до 24,
`light.startTimeMSK` → `00:00`. Раньше Light-окно было 06:00–20:00 МСК,
и ночью (20:00–06:00) статусы не проверялись — ночные пропадания камер
фиксировались только в первом утреннем прогоне. Камеры пишут и фиксируют
события и ночью, поэтому `reports/live.html` и `state/timeline-*.json`
теперь обновляются круглосуточно каждые 15 минут. Задача `AutoCamera Light`
в Планировщике пересоздана через `setup-schedule.ps1` (новый
`RepetitionDuration = PT24H`); строка «Zapusk:» в `menu.ps1` при
`durationHours >= 24` показывает «kruglosutochno (24/7)». Версия проекта
не поднята — изменено лишь значение уже-настраиваемого параметра
расписания (пункт `S → 3` в меню), функциональность та же.
